...
new_blurred = col_mat.dot(row_mat.dot(im_array.flatten()))
...
# plt.imshow(new_blurred.reshape((N+8,)*2), cmap=cm.Greys_r)
# plt.show()
...
GtG = np.transpose(G).dot(G)
print(GtG.shape)
Gty = np.transpose(G).dot(flat_blurry_image)
print(Gty.shape)
# GtGinv = np.linalg.inv(GtG)
# print(GtGinv.shape)
# GtGinvGt = GtGinv.dot(np.transpose(G))
# print(GtGinvGt.shape)
# print(flat_blurry_image.shape)
# print(G.shape)
# print(flat_blurry_image.shape)
...
# new_blurred = col_mat.dot(row_mat.dot(blurry_image.flatten()))
# plt.imshow(new_blurred.reshape((N+8,)*2), cmap=cm.Greys_r)
# plt.show()
